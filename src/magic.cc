/*
  Reference implementations of computing and using the "magic number" approach
  to dividing
  by constants, including codegen instructions. The unsigned division
  incorporates the
  "round down" optimization per ridiculous_fish.

  This is free and unencumbered software. Any copyright is dedicated to the
  Public Domain.
*/
...
// Bits in an sint
constexpr unsigned SINT_BITS = sizeof(my_sint) * CHAR_BIT;

// Bits in a uint
constexpr unsigned UINT_BITS = sizeof(my_uint) * CHAR_BIT;

/* Types used in the computations below. These can be redefined to the types
   appropriate
   for the desired division type (i.e. uint can be defined as unsigned long
   long).

   Note that the uint type is used in compute_signed_magic_info, so the uint
   type must
...
// typedef my_uint uint;
// typedef my_sint sint;

/* Computes "magic info" for performing signed division by a fixed integer D.
   The type 'sint' is assumed to be defined as a signed integer type large
  enough
...
struct magics_info compute_signed_magic_info(my_sint D);

/* Computes "magic info" for performing unsigned division by a fixed positive
  integer D.
   The type 'uint' is assumed to be defined as an unsigned integer type large
  enough
   to hold both the dividend and the divisor. num_bits can be set appropriately
  if n is
  ...
  which is
   not zero and not a power of 2, and a variable n of width num_bits (which may
  be
   up to UINT_BITS). To emit code for n/d, use one of the two following
  sequences
...
struct magicu_info compute_unsigned_magic_info(my_uint D, unsigned num_bits) {

  // The numerator must fit in a uint
  assert(num_bits > 0 && num_bits <= sizeof(my_uint) * CHAR_BIT);
  ...
  // work
  const my_uint initial_power_of_2 = (my_uint)1 << (UINT_BITS - 1);

  // The remainder and quotient of our power of 2 divided by d
  my_uint quotient = initial_power_of_2 / D, remainder = initial_power_of_2 % D;

  // ceil(log_2 D)
  unsigned ceil_log_2_D;

  // The magic info for the variant "round down" algorithm
  my_uint down_multiplier = 0;
  ...
  if (exponent < ceil_log_2_D) {
    ...
    assert(result.increment == 0 &&
           result.pre_shift ==
               0); // expect no increment or pre_shift in this path
...
struct magics_info compute_signed_magic_info(my_sint D) {
  ...
  // Absolute value of D (we know D is not the most negative value since that's
  // a power of 2)
  const my_uint abs_d = (D < 0 ? -D : D);

  // The initial power of 2 is one less than the first one that can possibly
  // work
  // "two31" in Warren
  unsigned exponent = SINT_BITS - 1;
  const my_uint initial_power_of_2 = (my_uint)1 << exponent;
  ...
  const my_uint tmp = initial_power_of_2 + (D < 0);
  const my_uint abs_test_numer = tmp - 1 - tmp % abs_d;

  // Initialize our quotients and remainders (q1, r1, q2, r2 in Warren)
  my_uint quotient1 = initial_power_of_2 / abs_test_numer,
          remainder1 = initial_power_of_2 % abs_test_numer;
  my_uint quotient2 = initial_power_of_2 / abs_d,
          remainder2 = initial_power_of_2 % abs_d;
  my_uint delta;
